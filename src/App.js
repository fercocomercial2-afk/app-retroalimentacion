...
/* ================================================================
   ...
   ================================================================ */
const RS_ESTADOS = {
  'VIGENTE': { color: '#2C8B5D', bg: '#f0fff4' },
  'EN TRÁMITE': { color: '#C98A2B', bg: '#fffbf0' },
  'PARA SOLICITUD': { color: '#C98A2B', bg: '#fffbf0' },
};
const RS_ESTADO_OPTIONS = ['VIGENTE', 'EN TRÁMITE', 'PARA SOLICITUD'];
...
const RS_SCHEMAS = {
  dm: {
    ...
    fields: [
      ...
      { key: 'estado', label: 'Estado', type: 'select', options: RS_ESTADO_OPTIONS },
...
function Sidebar({ user, screen, setScreen, isAdmin, onLogout, managerName, notifCount, hasRegistrosAccess, isRegistrosOnly }) {
  ...
  const items = isRegistrosOnly ? [
    ...
    { id: 'notificaciones', icon: '🔔', label: 'Notificaciones' },
    { id: 'reconocimientos', icon: '🏆', label: 'Reconocimientos' },
    ...(hasRegistrosAccess ? [{ id: 'registros', icon: '🧪', label: 'Registros' }] : []),
...
function getRsNotifications(rsDM, rsCosm, rsPF, rsDigesa, certDigemid) {
  ...
  for (const item of all) {
    ...
    if (dias === null || dias > 180) continue; // 6 meses = 180 días
    const schema = RS_SCHEMAS[item._tab];
    const tipo = dias < 0 ? 'vencido' : dias === 0 ? 'hoy' : dias <= 30 ? '1mes' : dias <= 60 ? '2meses' : '6meses';
    result.push({
      id: item.id, tab: item._tab, nombre: schema.nombre, titulo: item[schema.titleField],
      codigo: item[schema.codeField], diasRestantes: dias, tipo
    });
  }
...
function NotificacionesScreen({ myTasks, myOpportunities, allTasks, allOpportunities, allEmployees, isAdmin, adminView, setAdminView, onOpenInTrabajadores, rsDM, rsCosm, rsPF, rsDigesa, certDigemid, hasRegistrosAccess, onOpenInRegistros }) {
  ...
  const rsColor = (tipo) => tipo === 'vencido' ? '#D64545' : tipo === 'hoy' ? '#C98A2B' : tipo === '1mes' ? '#C98A2B' : tipo === '2meses' ? '#3A8F8F' : '#1F6FB2';
  ...
  const rsLabel = (n) => {
    ...
    if (n.tipo === '1mes') return `Vence en ${n.diasRestantes} días (menos de 1 mes)`;
    if (n.tipo === '2meses') return `Vence en ${n.diasRestantes} días (menos de 2 meses)`;
    return `Vence en ${n.diasRestantes} días (menos de 6 meses)`;
  };
  ...
  return (
    <div>
      ...
      {notifs.length === 0 ? (
        ...
        <div className="hist-list">
          {notifs.map(n => {
            const empFull = allEmployees.find(e => e.id === n.opp.employee_id);
            const wsp = empFull?.whatsapp;
            const msg = encodeURIComponent(`Hola ${n.emp?.name?.split(' ')[0] || ''}, te recuerdo que la tarea "${n.task.title}" del proyecto "${n.opp.title || n.opp.description}" ${n.type === 'hoy' ? 'vence HOY' : 'está vencida'}. Por favor, actualiza su estado en la plataforma. Gracias.`);
            return (
              <div key={n.id} className="hist-card" style={{ borderLeft: `4px solid ${n.type === 'hoy' ? '#C98A2B' : '#D64545'}` }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ fontSize: 22, cursor: 'pointer' }} onClick={() => onOpenInTrabajadores(n.opp, n.task)}>{n.type === 'hoy' ? '⚠️' : '🔴'}</div>
                  <div style={{ flex: 1 }} onClick={() => onOpenInTrabajadores(n.opp, n.task)}>
                  ...
                  {isAdmin && wsp && (
                    <a href={`https://api.whatsapp.com/send?phone=51${wsp}&text=${msg}`} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ flexShrink: 0, background: '#25D366', color: 'white', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 13, fontWeight: 700, cursor: 'pointer', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.122.558 4.112 1.528 5.835L.057 23.854c-.09.354.224.668.578.578l6.019-1.471A11.937 11.937 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-1.88 0-3.641-.522-5.143-1.428l-.366-.217-3.801.929.956-3.801-.233-.381A9.956 9.956 0 012 12C2 6.478 6.478 2 12 2s10 4.478 10 10-4.478 10-10 10z"/></svg>
                      WhatsApp
                    </a>
                  )}
                </div>
...
function RsAlertBadge({ fVencimiento }) {
  ...
  if (dias <= 30) return <span className="task-alert" title="Vence en menos de 1 mes"> ⚠️ Vence en {dias} días</span>;
  if (dias <= 60) return <span style={{ color: '#3A8F8F', fontWeight: 700 }} title="Vence en menos de 2 meses"> ⏳ Vence en {dias} días</span>;
  if (dias <= 180) return <span style={{ color: '#1F6FB2', fontWeight: 700 }} title="Vence en menos de 6 meses"> 📅 Vence en {dias} días</span>;
  return null;
}
...
function RegistrosSanitariosScreen({ rsTab, setRsTab, rsDM, rsCosm, rsPF, rsDigesa, certDigemid, rsAuditLog, canEdit, onOpenNuevo, onOpenEditar, onOpenEliminar }) {
      ...
      {items.length === 0 ? (
        ...
        <div className="hist-list">
          {items.map(item => {
            ...
            const estadoInfo = RS_ESTADOS[item.estado] || { color: '#888', bg: '#f5f5f5' };
            const historial = rsAuditLog ? rsAuditLog.filter(a => a.tabla === schema.table && a.record_id === item.id) : [];
            return (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 4, marginBottom: 4, flexWrap: 'wrap' }}>
                      {item.estado && (
                        <span style={{ background: estadoInfo.bg, color: estadoInfo.color, border: `1px solid ${estadoInfo.color}40`, borderRadius: 6, padding: '2px 10px', fontWeight: 700, fontSize: 12 }}>
                          {item.estado}
                        </span>
                      )}
                      <span className="hist-meta" style={{ margin: 0 }}>F. Vencimiento: {fmtDateRS(item.f_vencimiento)}</span>
                    </div>
                      ...
                      <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px solid #f0f0f0', display: 'grid', gap: 6 }}>
                        ...
                        {historial.length > 0 && (
                          <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed #e0e0e0' }}>
                            <div style={{ fontSize: 11, fontWeight: 700, color: '#999', marginBottom: 6, letterSpacing: '0.5px' }}>HISTORIAL DE CAMBIOS</div>
                            {historial.map(h => (
                              <div key={h.id} style={{ fontSize: 12, color: '#666', marginBottom: 5, lineHeight: 1.5 }}>
                                <span style={{ color: '#999' }}>{new Date(h.created_at).toLocaleString('es-PE', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
                                {' — '}<strong>{h.usuario_nombre || h.usuario_email || 'Usuario'}</strong>
                                {': '}{h.valor_nuevo || h.accion}
                              </div>
                            ))}
                          </div>
                        )}
...
/* ================================================================
   RECONOCIMIENTOS
   ================================================================ */
function ReconocimientosScreen({ recognitions, employees, opportunities }) {
  const empById = {};
  ...
  return (
    <div>
      <div className="page-header">
        ...
        <p className="page-subtitle">Reconocimientos otorgados a colaboradores</p>
      </div>
      {recognitions.length === 0 ? (
        <div className="empty-state">Aún no hay reconocimientos otorgados.</div>
      ) : (
        <div className="hist-list">
          {recognitions.map(r => {
            const emp = empById[r.employee_id];
            const opp = oppById[r.opportunity_id];
            return (
              <div key={r.id} className="hist-card" style={{ cursor: 'default', borderLeft: '4px solid #C98A2B' }}>
                <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                  <div style={{ fontSize: 22 }}>🏆</div>
                  <div style={{ flex: 1 }}>
                    <div className="hist-desc" style={{ fontWeight: 700 }}>{emp?.name || '—'}</div>
                    <div className="hist-meta">
                      {opp ? (opp.title || opp.description) : '—'} · {fmtDate(r.created_at)}
                    </div>
                    {r.comment && <div style={{ fontSize: 13, color: '#555', marginTop: 4, whiteSpace: 'pre-wrap' }}>"{r.comment}"</div>}
                    <div style={{ fontSize: 12, color: '#aaa', marginTop: 4 }}>Otorgado por: {r.given_by_name || r.given_by_email}</div>
                  </div>
                </div>
              </div>
            );
...
export default function App() {
  ...
  const loadAll = useCallback(() => {
    ...
    loadModulePermissions(); loadRsDM(); loadRsCosm(); loadRsPF(); loadRsDigesa(); loadCertDigemid(); loadRsAuditLog(); loadRecognitions();
  ...
  const openLograr = (opp) => {
    setPendingLograrOpp(opp);
  ...
  const continuarLograr = async () => {
    if (giveRecognition && recognitionComment.trim()) {
      ...
      await supabase.from('recognitions').insert([{
        ...
        given_by_email: user?.email || null,
        given_by_name: currentManager?.name || user?.email || 'Usuario',
      }]);
      loadRecognitions();
    }
    setRecognitionModal(false);
    setModal('lograr');
    setActiveOpp(pendingLograrOpp);
    setActiveTask(null);
    setFLograrObs('');
    setFormError('');
  };
  ...
  const eliminarTask = async () => {
    ...
    closeModal(); loadTasks(); loadOpportunities();
  };
  ...
  const isRegistrosOnly = hasRegistrosAccess && !isAdmin && user?.email === 'lrocca@ferco-medical.com';
  ...
  return (
    <div className="app-layout">
      ...
      <main className="main-content">
        ...
        {screen === 'registros' && hasRegistrosAccess && <RegistrosSanitariosScreen rsTab={rsTab} setRsTab={setRsTab} rsDM={rsDM} rsCosm={rsCosm} rsPF={rsPF} rsDigesa={rsDigesa} certDigemid={certDigemid} rsAuditLog={rsAuditLog} canEdit={canEditRegistros} onOpenNuevo={openRsNuevo} onOpenEditar={openRsEditar} onOpenEliminar={openRsEliminar} />}
        {screen === 'registros' && !hasRegistrosAccess && <div className="error-msg">No tienes acceso a esta sección</div>}
        {!isRegistrosOnly && screen === 'config' && isAdmin && <ConfigScreen employees={employees} categories={categories} onEmployeesUpdated={loadEmployees} onAssignmentsUpdated={loadAssignments} onCategoriesUpdated={loadCategories} />}
        {!isRegistrosOnly && screen === 'config' && !isAdmin && <div className="error-msg">No tienes acceso a esta sección</div>}
      </main>

      {/* MODAL: RECONOCIMIENTO (antes de Lograr) */}
      {recognitionModal && pendingLograrOpp && (
        <Modal onClose={() => setRecognitionModal(false)}>
          <div className="modal-title">¿Dar reconocimiento?</div>
          <div className="modal-context">
            <div className="modal-context-label">Oportunidad / Proyecto</div>
            <div className="modal-context-text">{pendingLograrOpp.title || pendingLograrOpp.description}</div>
            <div className="modal-context-meta">Colaborador: {employees.find(e => e.id === pendingLograrOpp.employee_id)?.name || '—'}</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <input type="checkbox" id="chkRecon" checked={giveRecognition} onChange={e => setGiveRecognition(e.target.checked)} style={{ width: 18, height: 18, cursor: 'pointer' }} />
            <label htmlFor="chkRecon" style={{ fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>🏆 Otorgar reconocimiento al colaborador</label>
          </div>
          {giveRecognition && (
            <div className="form-group">
              <label>Comentario del reconocimiento (opcional)</label>
              <textarea className="form-textarea" placeholder="¿Por qué merece este reconocimiento?" value={recognitionComment} onChange={e => setRecognitionComment(e.target.value)} />
            </div>
          )}
          <div className="modal-actions">
            <button className="btn-secondary" onClick={() => setRecognitionModal(false)}>Cancelar</button>
            <button className="btn-lograr-confirm" onClick={continuarLograr}>Continuar →</button>
          </div>
        </Modal>
      )}
